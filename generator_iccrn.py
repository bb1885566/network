...
class GeneratorICCRN(nn.Module):
    def __init__(self, m: int, n: int, channels = 20):
        """
        Generator of EBEN
        ...
        self.out_conv = nn.Conv2d(in_channels=channels * 3, out_channels=8, kernel_size=(1, 1), padding=(0, 0),
                                  bias=True)
    ...
    def forward(self, cut_audio):
        """
        Forward pass of the ICCRN generator module.

        Args:
            cut_audio (torch.Tensor): in-ear speech signal

        Returns:
            enhanced_spec_ri (torch.Tensor): The eight channels complex spectrum.
            enhanced_speech (torch.Tensor): The enhanced signal.
            enhanced_speech_time_split (torch.Tensor): The enhanced PQMF bands before the synthesis stage
        """

        first_bands = self.pqmf(cut_audio, "analysis", self.p)

        X0 = self.stft(first_bands)
        ...
        enhanced_spec_ri = self.out_conv(torch.cat([d0, d1], dim=1))


        enhanced_spec_ri[:, 0:2, :, :] = enhanced_spec_ri[:, 0:2, :, :] + X0
        enhanced_bands_list = []
        for i in range(4):
            j = 2 * i
            enhanced_speech_r = enhanced_spec_ri[:, j: j + 1, :, :]
            enhanced_speech_i = enhanced_spec_ri[:, j + 1: j + 2, :, :]
            enhanced_bands_list.append(torch.squeeze(self.istft(torch.cat([enhanced_speech_r, enhanced_speech_i],dim=1),t=first_bands.shape[-1]),dim=1))
            
            
        

        enhanced_speech_time_split = torch.stack(enhanced_bands_list, dim=1)
        
        enhanced_speech = torch.sum(
            self.pqmf(enhanced_speech_time_split, "synthesis"), 1, keepdim=True
        )


        return enhanced_spec_ri, enhanced_speech, enhanced_speech_time_split
